...
app.post('/clientes', (req, res) => {
  ...
  const query = 'INSERT INTO clientes (nombre, correo, telefono, direccionEnvio) VALUES (?, ?, ?, ?)';
  connection.query(query, [nombre, correo, telefono, direccionEnvio], (error, results) => {
    if (error) throw error;
...
app.put('/clientes/:id', (req, res) => {
  const { id } = req.params;
  const { nombre, correo, telefono, direccionEnvio } = req.body;
  const query = 'UPDATE clientes SET nombre = ?, correo = ?, telefono = ?, direccionEnvio = ? WHERE id = ?';
  connection.query(query, [nombre, correo, telefono, direccionEnvio, id], (error, results) => {
    if (error) throw error;
